...
This book is intended for a diﬀerent audience, and it has diﬀerent goals. I
...
goals is to help them learn C. For that part of the class, I use Griﬃths and Grif-
ﬁths, Head First C, from O’Reilly Media. This book is meant to complement
that one.
...
Chapter 1 explains some of the diﬀerences between compiled and interpreted
...
Chapter 4 is about ﬁle systems and data streams. Recommended reading:
...
text, I have not yet included the ﬁgures. So there are a few places where, I’m
sure, the explanation will be greatly improved when the ﬁgures are ready.

0.1 Using the code

Example code for
this book is available from https://github.com/
AllenDowney/ThinkOS. Git is a version control system that allows you to
keep track of the ﬁles that make up a project. A collection of ﬁles under
...
(cid:136) You can create a copy of my repository on GitHub by pressing the Fork
...
(cid:136) Andy Engle knows the diﬀerence between GB and GiB.

(cid:136) Aashish Karki noted some broken syntax.
...
Representing ﬂoating-point numbers . . . . . . . . . . . . . .
...
guage; nevertheless, there are some general diﬀerences between compiled and
...
diﬀerent types. Any values that support the addition operator will work; any
...
The ﬁrst line of the function includes type declarations for the parameters
...
directives that take eﬀect before the program is compiled. For example,
the #include directive causes the source code from another ﬁle to be
...
5. Linking: If the program uses values and functions deﬁned in a library,
the compiler has to ﬁnd the appropriate library and include the required
code.

1This is a simpliﬁcation; we will go into more detail later.
...
cutable ﬁle. For example, here is a minimal C program:
...
If you save this code in a ﬁle called hello.c, you can compile and run it like
this:

$ gcc hello.c
$ ./a.out

By default, gcc stores the executable code in a ﬁle called a.out (which origi-
nally stood for “assembler output”). The second line runs the executable. The
preﬁx ./ tells the shell to look for it in the current directory.

It is usually a good idea to use the -o ﬂag to provide a better name for the
executable:
...
The -c ﬂag tells gcc to compile the program and generate machine code, but
not to link it or generate an executable:

$ gcc hello.c -c

The result is a ﬁle named hello.o, where the o stands for object code, which
is the compiled program. Object code is not executable, but it can be linked
into an executable.

The UNIX command nm reads an object ﬁle and generates information about
the names it deﬁnes and uses. For example:
...
This output indicates that hello.o deﬁnes the name main and uses a function
...
You can control how much optimization gcc does with the -O ﬂag. By default,
...
than to speed it up. But if your program has a subtle bug, you might ﬁnd that
optimization makes the bug appear or disappear. It is usually a good idea to
turn oﬀ optimization while you are developing new code. Once the program
is working and passing appropriate tests, you can turn on optimization and
conﬁrm that the tests still pass.

1.5 Assembly code

Similar to the -c ﬂag, the -S ﬂag tells gcc to compile the program and generate
assembly code, which is basically a human-readable form of machine code.

$ gcc hello.c -S

The result is a ﬁle named hello.s, which might look something like this:
...
gcc is usually conﬁgured to generate code for the machine you are running on,
so for me it generates x86 assembly language, which runs on a wide variety
of processors from Intel, AMD, and others. If you are running on a diﬀerent
architecture, you might see diﬀerent code.

1.6 Preprocessing

Taking another step backward through the compilation process, you can use
the -E ﬂag to run the preprocessor only:

$ gcc hello.c -E
The result is the output from the preprocessor. In this example, it contains
the included code from stdio.h, and all the ﬁles included from stdio.h, and
all the ﬁles included from those ﬁles, and so on. On my machine, the total is
...
If you use a function that’s not deﬁned in any of the standard libraries, you
...
only a few runtime errors you are likely to see.
If you divide by zero, or
perform another illegal ﬂoating-point operation, you will get a “Floating point
...
Before we talk about processes, I want to deﬁne a few words:

(cid:136) Abstraction: An abstraction is a simpliﬁed representation of something
...
a notiﬁcation when it is available for pickup. I don’t need to know where
...
with many other virtual machines, on a computer running a diﬀerent operating
system.
...
undesired eﬀects on other components.
...
have diﬀerent data and hardware states.
...
(cid:136) Device abstraction: Processes running on the same computer share the
...
are implemented. But if you are curious, you will ﬁnd a lot of interesting
...
up the shell.
If I type make in the shell, it creates a new process to run
...
The ﬁrst column is the unique numerical process ID. The second column is
...
If you use the -e ﬂag, you get every process (including processes belonging to
other users, which is a security ﬂaw, in my opinion).
...
init is the ﬁrst process created when the operating system starts. It creates
...
extra d at the end stands for daemon, which is another name for processes like
...
speciﬁcally, it handles software interrupt requests, or “soft IRQ”.

kworker is a worker process created by the kernel to do some kind of processing
for the kernel.
...
Suppose I ﬂip a coin and tell you the outcome. I have given you one bit of
information. If I roll a six-sided die and tell you the outcome, I have given you
log26 bits of information. And in general, if the probability of the outcome is
1 in N , then the outcome contains log2N bits of information.

Equivalently, if the probability of the outcome is p, then the information con-
tent is −log2p. This quantity is called the self-information of the outcome.
...
is something you were already conﬁdent of, conﬁrming it contributes only a
...
If the process reads and writes ﬁles, those ﬁles are usually stored on a hard
...
well as RAM, but the properties of these devices are very diﬀerent, so we will
need to distinguish them. I will use storage to refer to HDDs and SSDs.

3.3 Address spaces

Each byte in main memory is speciﬁed by an integer physical address. The
...
1,073,741,823 in decimal, or 0x3ﬀf ﬀﬀ in hexadecimal (the preﬁx 0x indicates
...
the virtual address space runs from 0 to 0xﬀﬀ ﬀﬀ. The size of this address
space is 232 bytes, or 4 GiB.
...
they would map to diﬀerent locations in physical memory.
...
The data of a running process is organized into ﬁve segments:

(cid:136) The code segment contains the program text; that is, the machine

language instructions that make up the program.

(cid:136) The static segment contains immutable values, like string literals. For
...
(cid:136) The text segment is near the “bottom” of memory, that is, at addresses

near 0.

(cid:136) The static segment is often just above the text segment, that is, at higher

addresses.

(cid:136) The global segment is often just above the static segment.
...
program, which is in aspace.c in the repository for this book (see Section 0.1).
...
address of the ﬁrst machine language instruction in main, which we expect to
...
program (as opposed to a string that is read from a ﬁle, input by a user, etc.).
...
As expected, the address of main is the lowest, followed by the location of the
string literal. The location of global is next, then the address p points to.
...
If you add this function to aspace.c you can conﬁrm that counter is allocated
...
CPUMMUMCUvirtual addressphysical addressvirtual page #virtual addressphysical addressphysical page #offsetoffsetTLBMMU3.6. Address translation
...
oﬀset. A “page” is a chunk of memory; the size of a page depends on
the operating system and the hardware, but common sizes are 1–4 KiB.

3. The MMU looks up the page number in the translation lookaside buﬀer
(TLB) and gets the corresponding physical page number. Then it com-
bines the physical page number with the oﬀset to produce a PA.
...
(cid:136) Since 1 GiB is 230 bytes and 1 KiB is 210 bytes, there are 220 physical
...
(cid:136) The size of the oﬀset is determined by the page size. In this example the

page size is 210 B, so it takes 10 bits to specify a byte on a page.

(cid:136) If a VA is 32 bits and the oﬀset is 10 bits, the remaining 22 bits make

up the virtual page number.

(cid:136) Since there are 220 physical pages, each physical page number is 20 bits.

Adding in the 10 bit oﬀset, the resulting PAs are 30 bits.
...
would need 232 bytes, or 4 GiB, just for page tables! And that’s just with 32-bit
...
org/wiki/Page_table; you might ﬁnd the details interesting. But the fun-
...
quentially, but they raise a diﬀerent complication: each block can be written
...
common abstraction is called a “ﬁle system.”

Abstractly:

(cid:136) A “ﬁle system” is a mapping from each ﬁle’s name to its contents. If
you think of the names as keys, and the contents as values, a ﬁle system
is a kind of key-value database (see https://en.wikipedia.org/wiki/
Key-value_database).

(cid:136) A “ﬁle” is a sequence of bytes.

File names are usually strings, and they are usually “hierarchical”; that is, the
string speciﬁes a path from a top-level directory (or folder), through a series
of subdirectories, to a speciﬁc ﬁle.

24

Chapter 4. Files and ﬁle systems

The primary diﬀerence between the abstraction and the underlying mechanism
is that ﬁles are byte-based and persistent storage is block-based. The operating
system translates byte-based ﬁle operations in the C library into block-based
operations on storage devices. Typical block sizes are 1–8 KiB.

For example, the following code opens a ﬁle and reads the ﬁrst byte:
...
1. fopen uses the ﬁlename to ﬁnd the top-level directory, called /, the

subdirectory home, and the sub-subdirectory downey.

2. It ﬁnds the ﬁle named file.txt and “opens” it for reading, which means
it creates a data structure that represents the ﬁle being read. Among
other things, this data structure keeps track of how much of the ﬁle has
been read, called the “ﬁle position”.
...
there seems to be no good name for it. It is an entry in the open ﬁle
table, so I will call it an OpenFileTableEntry.

3. When we call fgetc, the operating system checks whether the next char-
acter of the ﬁle is already in memory. If so, it reads the next character,
advances the ﬁle position, and returns the result.
...
memory, and the process resumes. It reads the ﬁrst character and stores
it as a local variable.

6. When the process closes the ﬁle, the operating system completes or can-
cels any pending operations, removes data stored in memory, and frees
the OpenFileTableEntry.

The process for writing a ﬁle is similar, but there are some additional steps.
Here is an example that opens a ﬁle for writing and changes the ﬁrst character.
...
1. Again, fopen uses the ﬁlename to ﬁnd the ﬁle. If it does not already
exist, it creates a new ﬁle and adds an entry in the parent directory,
/home/downey.

2. The operating system creates an OpenFileTableEntry that indicates that

the ﬁle is open for writing, and sets the ﬁle position to 0.

3. fputc attempts to write (or re-write) the ﬁrst byte of the ﬁle. If the
ﬁle already exists, the operating system has to load the ﬁrst block into
memory. Otherwise it allocates a new block in memory and requests a
new block on disk.

4. After the block in memory is modiﬁed, it might not be copied back to
the disk right away. In general, data written to a ﬁle is “buﬀered”, which
means it is stored in memory and only written to disk when there is at
least one block to write.

5. When the ﬁle is closed, any buﬀered data is written to disk and the

OpenFileTableEntry is freed.

To summarize, the C library provides the abstraction of a ﬁle system that
maps from ﬁle names to streams of bytes. This abstraction is built on top of
...
ms (see https://en.wikipedia.org/wiki/Hard_disk_drive_performance_
...
hardware provide several features intended to “ﬁll in” this gap:
...
if you open a ﬁle and read the ﬁrst block, there is a good chance you
will go on to read the second block. The operating system might start
loading additional blocks before they are requested.

(cid:136) Buﬀering: As I mentioned, when you write a ﬁle, the operating system
...
the problem, and (2) when data is buﬀered, it can be harder to debug a
program. For example, if a program prints a value and then crashes, the value
might not appear, because it might be in a buﬀer. Similarly, if a program
...
The blocks that make up a ﬁle might be arranged contiguously on disk, and ﬁle
...
In many UNIX ﬁle systems, that data structure is called an “inode,” which
stands for “index node”. More generally, information about ﬁles, including
the location of their blocks, is called “metadata”. (The content of the ﬁle is
data, so information about the ﬁle is data about data, hence “meta”.)

Since inodes reside on disk along with the rest of the data, they are designed
to ﬁt neatly into disk blocks. A UNIX inode contains information about a
ﬁle, including the user ID of the ﬁle owner; permission ﬂags indicating who
is allowed to read, write, or execute it; and timestamps that indicate when it
was last modiﬁed and accessed. In addition, it contains block numbers for the
ﬁrst 12 blocks that make up the ﬁle.

If the block size is 8 KiB, the ﬁrst 12 blocks make up 96 KiB. On most systems,
that’s big enough for a large majority of ﬁles, but it’s deﬁnitely not big enough
...
for all but the largest ﬁles, but still not big enough for all.
...
And if that’s not big enough, there is (ﬁnally) a triple indirection block, which
contains pointers to double indirection blocks, yielding a maximum ﬁle size of
8 TiB. When UNIX inodes were designed, that seemed big enough to serve for
a long time. But that was a long time ago.

As an alternative to indirection blocks, some ﬁles systems, like FAT, use a File
Allocation Table that contains one entry for each block, called a “cluster” in
this context. A root directory contains a pointer to the ﬁrst cluster in each ﬁle.
The FAT entry for each cluster points to the next cluster in the ﬁle, similar to
...
File systems have to keep track of which blocks belong to each ﬁle; they also
have to keep track of which blocks are available for use. When a new ﬁle is
created, the ﬁle system ﬁnds an available block and allocates it. When a ﬁle
is deleted, the ﬁle system makes its blocks available for re-allocation.
...
It is hard to design a ﬁle system that achieves all of these goals, especially
since ﬁle system performance depends on “workload characteristics” like ﬁle
sizes, access patterns, etc. A ﬁle system that is well tuned for one workload
might not perform as well for another.

For this reason, most operating systems support several kinds of ﬁle systems,
and ﬁle system design is an active area of research and development. In the
last decade, Linux systems have migrated from ext2, which was a conventional
UNIX ﬁle system, to ext3, a “journaling” ﬁle system intended to improve speed
and contiguity, and more recently to ext4, which can handle larger ﬁles and
ﬁle systems. Within the next few years, there might be another migration to
the B-tree ﬁle system, Btrfs.

4.4 Everything is a ﬁle?

The ﬁle abstraction is really a “stream of bytes” abstraction, which turns out
to be useful for many things, not just ﬁle systems.
...
as input into another process. For the ﬁrst process, the pipe behaves like a
ﬁle open for writing, so it can use C library functions like fputs and fprintf.

4.4. Everything is a ﬁle?

29

For the second process, the pipe behaves like a ﬁle open for reading, so it uses
fgets and fscanf.
...
processes on diﬀerent computers (usually). Again, processes can read data
from and write data to a socket using “ﬁle” handling functions.

Reusing the ﬁle abstraction makes life easier for programmers, since they only
have to learn one API (application program interface). It also makes programs
more versatile, since a program intended to work with ﬁles can also work with
...
To ﬁnd the two’s complement of a negative number, −x, ﬁnd the binary rep-
resentation of x, ﬂip all the bits, and add 1. For example, to represent −510,
start with the representation of 510, which is b00000101 if we write the 8-bit
...
positive number and add 1’s for a negative number. In eﬀect, we have to copy
...
bers) unless you declare them unsigned. The diﬀerence, and the reason this
...
set bits, and ^ is used to ﬂip, or “toggle” bits. Here are the details:
...
5.3. Representing ﬂoating-point numbers
...
Toggling bits: Finally, if you XOR a vector with 3, it ﬂips the rightmost bits
...
Floating-point numbers are represented using the binary version of scientiﬁc
notation. In decimal notation, large numbers are written as the product of a
coeﬃcient and 10 raised to an exponent. For example, the speed of light in
m/s is approximately 2.998 · 108.

Most computers use the IEEE standard for ﬂoating-point arithmetic. The C
...
are the exponent, q, and the last 23 bits are the coeﬃcient, c. The value of a
ﬂoating-point number is
...
For example, the integer representation of 1310 is b1101.
In ﬂoating point,
that’s 1.101 · 23, so the exponent is 3 and the part of the coeﬃcient that would
...
To pack and unpack ﬂoating-point numbers in C, we can use a union and
...
The union allows us to store a ﬂoating-point value using p.f and then read it
...
To get the coeﬃcient, we need to extract the 23 rightmost bits and ignore the
...
including the bias. And the coeﬃcient, which I printed in hexadecimal, is 101
followed by 20 zeros.

As an exercise, try assembling or disassembling a double, which uses the 64-bit
...
that might be an integer, ﬂoat, complex, or rational number.

However, unions are error-prone. It is up to you, as the programmer, to keep
track of what type of data is in the union; if you write a ﬂoating-point value
...
stack and ﬁlls it with the numbers from 0 to 99.
...
Next I’ll deﬁne a function that creates a smaller array and deliberately accesses
...
In this example, all of the variables are integers, so it is relatively easy to ﬁgure
...
to make an array of ﬂoats, the results are:
...
The latter two values are what you get if you interpret a ﬂoating-point value
as an integer. If you encountered this output while debugging, you would have
a hard time ﬁguring out what’s going on.
...
A careful observer will notice that they diﬀer by a single bit. And this pattern
...
case to upper-case by ﬂipping the sixth bit. As a challenge, you can make a
...
Conversely, if you write a string and then accidentally read it as an int or ﬂoat,
...
You will ﬁnd that the ASCII representation of the ﬁrst 8 characters
is
of my name,
69779713878800585457664.

interpreted as a double-precision ﬂoating point number,
...
(cid:136) realloc, which takes a pointer to a previously allocated chunk and a
...
It might not sound diﬃcult to follow these rules, but in a large program a chunk
...
in diﬀerent parts of the program. The chunk should not be freed until all
references to the chunk are no longer in use. Getting this right often requires
careful analysis across all parts of the program, which is diﬃcult and contrary
...
To make matters worse, memory errors can be diﬃcult to ﬁnd because the
...
be diﬀerent each time the program runs.
...
At that point it will be very diﬃcult to ﬁnd the source of the problem.
...
references to the same chunk in diﬀerent parts of your program.

Often there is a trade-oﬀ between safe memory management and performance.
...
will increase indeﬁnitely. At that point, a few things might happen:
...
(cid:136) On systems with virtual memory, the operating system can move another
...
(cid:136) Eventually, a process might ﬁll its virtual address space (or the usable
...
error condition. Sometimes other codes are used to indicate diﬀerent error
conditions.
...
Because memory management is so diﬃcult, most large programs, like web
...
When malloc is called, it checks whether it can ﬁnd a free chunk that’s big
...
patterns, it runs fast and uses memory eﬃciently.
...
(cid:136) The run time of malloc does not usually depend on the size of the chunk,
...
chunks, malloc is not space eﬃcient.
If your program requires large
numbers of small structures, it might be more eﬃcient to allocate them
in arrays.
...
the program down by making memory caches less eﬀective.
...
(cid:136) A status register, or ﬂag register, that contains information about the
current computation. For example, the ﬂag register usually contains a
...
Most computers can execute a few hundred diﬀerent instructions, called the
“instruction set”. But most instructions fall into a few general categories:

(cid:136) Load: Transfers a value from memory to a register.

(cid:136) Arithmetic/logic: Loads operands from registers, performs a mathemat-

ical operation, and stores the result in a register.

(cid:136) Store: Transfers a value from a register to memory.

(cid:136) Jump/branch: Changes the program counter, causing the ﬂow of execu-
tion to jump to another location in the program. Branches are usually
conditional, which means that they check a ﬂag in the ﬂag register and
...
The performance of many programs is limited by the eﬀectiveness of the cache.
...
The cache “hit rate”, h, is the fraction of memory accesses that ﬁnd data in
...
Equivalently, we could deﬁne the “miss penalty” as the extra time to process
...
When a program reads a byte for the ﬁrst time, the cache usually loads a
...
64, and the ﬁrst byte of the string happens to fall at the beginning of a block.
When you load the ﬁrst byte, you incur a miss penalty, but after that the rest
...
(cid:136) Most programs contain blocks of code with no jumps or branches. Within
...
(cid:136) When a program executes a function, its parameters and local variables
...
My modiﬁed version of this program is in the cache directory of the repository
...
the average time with suﬃcient precision.
...
The second loop runs the same number of iterations as the ﬁrst. After each
...
to increment temp. This diﬀerence is the total miss penalty incurred by all
...
8 read+write: 0.8633 ns
16 read+write: 0.7023 ns
32 read+write: 0.7105 ns
64 read+write: 0.7058 ns
...
212214216218220222224226size (B)0246810average penalty (ns)512 B256 B128 B64 B32 B16 B8 B7.5. Programming for cache performance

51

(cid:136) The program reads through the array many times, so it has plenty of
temporal locality. If the entire array ﬁts in cache, we expect the average
...
the array does not ﬁt in cache.

(cid:136) If the stride is equal to the block size (or greater), the spatial locality is
eﬀectively zero, because each time we read a block, we only access one
...
a little when the array size is bigger than 214 B, so it’s possible that this
...
write programs that use them more eﬀectively.
...
drawback of cache-aware algorithms is that they are hardware-speciﬁc.
...
The trade-oﬀ between speed, size, and cost is the fundamental reason for
...
a word is 64 bits or 8 B. So the total size of the register ﬁle is 100–300 B.
...
For the other numbers in the table, I looked at the speciﬁcations for typical
...
(cid:136) Who moves data up and down the hierarchy? At the top of the hierarchy,
...
memory when they execute programs and open ﬁles. But the operating
...
system reads a ﬁle from disk, it might read 10s or 100s of blocks at a
time.

(cid:136) When does data get moved? In the most basic cache, data gets moved
into cache when it is used for the ﬁrst time. But many caches use some
kind of “prefetching”, meaning that data is loaded before it is explicitly
requested. We have already seen one form of prefetching:
loading an
...
memory to disk, then it modiﬁes the page table for Process B to indicate
...
(cid:136) If a program leaks memory, it might leave allocated space behind and
never access it again. By swapping those pages out, the operating system
can eﬀectively plug the leak.
...
event that stops the normal instruction cycle and causes the ﬂow of execution
...
the condition can be handled by the operating system. Some ﬂoating-point
...
interrupt, causing the ﬂow of execution to jump to the kernel. The kernel
...
the program counter of the interrupted process, which has the eﬀect of
...
(cid:136) Ready, if the process could be running, but isn’t, usually because there

are more runnable processes than cores.

(cid:136) Blocked, if the process cannot run because it is waiting for a future event

like network communication or a disk read.

(cid:136) Done, if the process has completed, but has exit status information that
...
The interrupt handler ﬁgures out which process was waiting for the re-
...
(cid:136) Processes might be limited by diﬀerent resources. A process that does
...
20 minutes to prepare the ﬁlling. If you prepare the crust ﬁrst, you can
prepare the ﬁlling while the crust is chilling, and you can ﬁnish the pie in
35 minutes. If you prepare the ﬁlling ﬁrst, the process takes 55 minutes.
...
(cid:136) A process usually starts with a relatively high priority so it starts running
...
a substantial eﬀect on performance. Simple scheduling policies are usually
good enough.
...
applications, a general-purpose operating system like Linux can be modiﬁed
to handle real-time scheduling. These modiﬁcations might include:
...
ority runs within a ﬁxed amount of time.

(cid:136) Reorganizing interrupt handlers to guarantee a maximum completion

time.

(cid:136) Modifying locks and other synchronization mechanisms (coming up in
...
the same code. But diﬀerent threads often run diﬀerent parts of the code.
...
Pthreads for short. The POSIX standard deﬁnes a thread model and an
interface for creating and controlling threads. Most versions of UNIX provide
an implementation of Pthreads.

Using Pthreads is like using most C libraries:

(cid:136) You include headers ﬁles at the beginning of your program.

(cid:136) You write code that calls functions deﬁned by Pthreads.
...
This compiles a source ﬁle named array.c with debugging info and opti-
...
ond, Shared is a structure I deﬁned to contain values shared between threads.
...
ﬁnd this example in counter/counter.c. You can compile and run it like
this:
...
When you run it, you will probably get diﬀerent results. And if you run it
again, you might get diﬀerent results each time. What’s going on?
...
Each time you run the program, threads might be interrupted at diﬀerent
points, or the scheduler might choose diﬀerent threads to run, so the sequence
of events, and the results, will be diﬀerent.

3

Suppose we want to impose some order. For example, we might want each
thread to read a diﬀerent value of counter and increment it, so that the value
of counter reﬂects the number of threads that have executed child_code.
...
I’ll show you how to use it ﬁrst; then I’ll explain how it works.
...
has the eﬀect of barring all other threads. Suppose Thread A has locked the
...
proceed. In eﬀect, the threads line up to execute child_code one at a time,
...
And that satisﬁes the requirements. In order for this solution to work, I have
...
The code in this section is in counter_mutex.c. The deﬁnition of Mutex is in
mutex.c, which I explain in the next section.

9.5 Mutex

My deﬁnition of Mutex is a wrapper for a type called pthread_mutex_t, which
is deﬁned in the POSIX threads API.
...
My code makes it easier to get that right. It deﬁnes a type, Mutex, which is
...
Then it deﬁnes make_mutex, which allocates space and initializes the mutex:
...
This code is in mutex.c and the header ﬁle mutex.h.
...
In some multi-threaded programs, threads are organized to perform diﬀerent
...
wrong and ﬁx it. The code for this example is in the repository for this book,
in a folder called queue. The ﬁle queue.c contains a basic implementation of
a circular buﬀer, which you can read about at https://en.wikipedia.org/
...
Here’s the structure deﬁnition:
...
make_queue allocates space for this structure and initializes the ﬁelds:
...
If we keep adding elements to the
that’s where we run into a tricky part.
...
To avoid that, we deﬁne another special case to indicate that the queue is full:
...
And ﬁnally here’s the shared structure that contains the queue:
...
(cid:136) Access to the queue is not thread safe. Diﬀerent threads could access
array, next_in, and next_out at the same time and leave the queue in
a broken, “inconsistent” state.

(cid:136) If the consumer is scheduled ﬁrst, it ﬁnds the queue empty, print an error
...
In the next section, we solve the ﬁrst problem with a Mutex. In the following
...
is required to lock the mutex ﬁrst; this requirement is part of the documented
...
I’ll handle the ﬁrst condition here, and you will have a chance to handle the
...
Now in queue_pop, if we ﬁnd the queue empty, we don’t exit; instead we use
...
cond_wait is complicated, so let’s take it slow. The ﬁrst argument is the
...
there are no threads waiting on the condition variable, the signal has no eﬀect.
...
In the next section I’ll show you how my Cond code works, but ﬁrst I want to
...
called pthread_cond_t, which is deﬁned in the POSIX threads API. It is very
similar to Mutex, which is a wrapper for pthread_mutex_t. Both wrappers
are deﬁned in utils.c and utils.h.
...
making it easier to work with. And it presents a ﬁnal challenge: can you write
...
The POSIX standard speciﬁes an interface for semaphores; it is not part of
...
to make it easier to use. The interface is deﬁned in sem.h:
...
Semaphore is a synonym for sem_t, but I ﬁnd it more readable, and the capital
...
Here’s the new deﬁnition of Queue, replacing the mutex and condition variables
with semaphores:
...
ables and mutexes. In the repository for this book, you’ll ﬁnd my solution in
mysem_soln.c and mysem_soln.h.

If you have trouble getting started, you can use the following structure deﬁni-
...
do...while loop. Can you ﬁgure out why it is not a more conventional while
...
It turns out that a well-timed spurious wakeup (see http://en.
